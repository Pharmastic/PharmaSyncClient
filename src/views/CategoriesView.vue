...
import { categorySchema } from '../validation/categorySchema.ts';
...
const headers = computed(() => {
  return [
    ...
    { title: 'Products Count', value: '_count.products' },
  ];
});
...
const onSubmit = handleSubmit(async (data) => {
  if (selectedCategory.value) {
    await updateCategory(selectedCategory.value.item.id, data);
...
const handleDelete = async () => {
  if (selectedCategory.value) {
    await deleteCategory(selectedCategory.value.item.id);
    await refreshCategories();
    showModal.value = false;
  }
};
      ...
      <div class="bg-white rounded-lg shadow-xl w-full max-w-md mx-4">
        <form @submit.prevent="onSubmit" class="flex flex-col gap-4">
          ...
          <div class="p-4 space-y-4">
            <TextField name="name" :value="selectedCategory.item.name" label="Name" required />
            <TextField
              name="description"
              :value="selectedCategory.item.description"
              label="Description"
              required
            />
          </div>
